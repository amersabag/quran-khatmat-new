Meteor.methods({
  ...
  setOwner: function(partId) {
    ...
    if (
        ...
        || !(user = Meteor.user())
        || (part.ownerId && !user.isAdmin)
    ) {
      throw new Meteor.Error(CONSTANTS.PartIdIsNotValidError);
    }
    Parts.update(
        {_id: partId},
        {$set: {ownerId: this.userId, ownerUsername: user.username}}
    );
    return true;
  },
  removeOwner: function(partId) {
    ...
    Parts.update(
        {_id: partId},
        {$set: {ownerId: null, ownerUsername: null}}
    );
    return true;
  },
  ...
  getKhatmaDueParts: function(khatmaId) {
    if (
        !Match.test(khatmaId,String)
        || !Meteor.userId()
        || !Khatmat.findOne({_id: khatmaId, createdBy: Meteor.userId()})
    ) {
      throw new Meteor.Error(CONSTANTS.DueReportError);
    }
    return Parts.find({
      khatmaId: khatmaId,
      $and: [{ownerId: {$exists: true}}, {ownerId: {$ne:  null}}],
      done: {$ne: true}
    },{
      fields: {
        periodId: 1,
      ...
      sort: {periodStartDate: 1, partNumber: 1}
    }).fetch();
  }
});
